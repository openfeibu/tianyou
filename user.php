...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        {
            ajax_show_message($_LANG['passport_js']['agreement']);
        }
        if (strlen($username) < 3)
        {
            ajax_show_message($_LANG['passport_js']['username_shorter']);
        }
        ...
        {
            ajax_show_message($_LANG['passwd_balnk']);
        }
        ...
        if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
        {
            ...
            {
                ajax_show_message($_LANG['invalid_captcha'],'invalid_captcha');
            }
		...
		{
			ajax_show_message($_LANG['mobile_phone_changed']);
		}
		// 检查验证码是否正确
        ...
        if (register($username, $password, $mobile, $other) !== false)
        {
            /*把新注册用户的扩展信息插入数据库*/
            ...
            ajax_show_message(sprintf($_LANG['register_success'], $username . $ucdata),'success');
        }
        else
        {
            ajax_show_message($GLOBALS['err']->_message['0']);
        }
    }
}
...
elseif ($action == 'act_login')
{
    ...
    $username = $user ? $user_data['user_name'] : '';
...
elseif ($action == 'ajax_act_login')
{
    ...
    if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($_POST['captcha']))
        {
            ajax_show_message($_LANG['invalid_captcha']);
        }
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        $ucdata = isset($user->ucdata)? $user->ucdata : '';
        ajax_show_message($_LANG['login_success'] . $ucdata , 'success');
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
        ajax_show_message($_LANG['login_failure']);
    }
}
/* 处理 ajax 的登录请求 */
...
elseif ($action == 'act_edit_profile')
{
    ...
    {
        ajax_show_message($_LANG['passport_js']['office_phone_invalid']);
    }
    if (!empty($home_phone) && !preg_match( '/^[\d|\_|\-|\s]+$/', $home_phone) )
    {
         ajax_show_message($_LANG['passport_js']['home_phone_invalid']);
    }
    if (!empty($email) && !is_email($email))
    {
        ajax_show_message($_LANG['msg_email_format']);
    }
    if (!empty($msn) && !is_email($msn))
    {
         ajax_show_message($_LANG['passport_js']['msn_invalid']);
    }
    if (!empty($qq) && !preg_match('/^\d+$/', $qq))
    {
         ajax_show_message($_LANG['passport_js']['qq_invalid']);
    }
    if (!empty($mobile_phone) && !preg_match('/^[\d-\s]+$/', $mobile_phone))
    {
        ajax_show_message($_LANG['passport_js']['mobile_phone_invalid']);
    }
    ...
    {
        ajax_show_message($_LANG['edit_profile_success'],'success');
    }
    else
    {
...
elseif($action == 'act_upload_avatar')
{
    ...
    {
        /* 用户没有登录 */
        ajax_show_message("请先登录！",'no_login');
    }
    ...
    if (isset($_FILES['avatar']['error'])) // php 4.2 版本才支持 error
    {
        ...
        {
            ajax_show_message(sprintf('图片文件太大了(最大值：1M)，无法上传。', $php_maxsize));
        }
        elseif ($_FILES['avatar']['error'] == 2)
        {
            ajax_show_message(sprintf('图片文件太大了(最大值：1M)，无法上传。', $htm_maxsize));
        }
    ...
    if (!empty($_FILES['avatar']['name']))
    {
        ...
        $avatar = $image->make_thumb($original_img, 190, '', $target);

        if ($avatar === false)
        {
            ajax_show_message("图片保存出错！");
        }
    }
    ...
    if (edit_profile($profile))
    {
        ajax_show_message($_LANG['edit_profile_success']);
    }
    else
    {
        if ($user->error == ERR_EMAIL_EXISTS)
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
			...
			$db->query($sql);
            $user->logout();
            ajax_show_message($_LANG['edit_password_success'],'success');
        }
        else
        {
            ajax_show_message($_LANG['edit_password_failure'],'success');
        }
    }
    else
    {
...
elseif ($action == 'act_edit_address')
{
    ...
    if(isset($_POST['district']) && intval($_POST['district']))
    {
        $address['district'] = intval($_POST['district']);
    }
    if(isset($_POST['is_default']) && intval($_POST['is_default']))
    {
...
elseif($action == 'act_edit_address_default')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    include_once(ROOT_PATH . 'includes/cls_json.php');
...
/* 删除收货地址 */
elseif ($action == 'ajax_drop_consignee')
{
    include_once('includes/lib_transaction.php');
    include_once('includes/cls_json.php');
    $json = new JSON;

    $consignee_id = intval($_POST['id']);
    if ($user_id == 0)
    {
        /* 用户没有登录 */
        $result['error']   = 1;
...
/* 发起申请退货 */
elseif ($action == 'order_back_request')
{
    $id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
    $goods_id = isset($_GET['goods_id']) ? intval($_GET['goods_id']) : 0;
    include_once(ROOT_PATH .'includes/lib_transaction.php');
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $order = get_order_detail($id, $user_id);
    $back_sn = strtotime(date('Y-m-d H:i:s',time()))+(rand()*100);
    $date = date('Y-m-d H:i:s',time());
    $invoice_no = $order['invoice_no'];
    $order_sn = $order['order_sn'];
    $back_sn = $db->getRow("SELECT back_sn FROM " . $ecs->table('order_back') . " WHERE order_sn = '$order_sn'");
    $goods = $db->getRow("SELECT og.goods_name,og.goods_price,og.goods_number,g.goods_thumb FROM " . $ecs->table('order_goods') . " AS og LEFT JOIN   ".$ecs->table('goods')." AS g ON og.goods_id = g.goods_id WHERE og.order_id = '$id' AND og.goods_id = '$goods_id' ");
    if($goods){
...
/* 寄送退货商品 */
elseif ($action == 'back_address_edit')
{
...
/* 提交退货配送信息表单 */
elseif ($action == 'order_back_request_submit')
{
...
elseif($action == 'act_upload_img')
{
    ...
    /* 检查图片：如果有错误，检查尺寸是否超过最大值；否则，检查文件类型 */
    if (isset($_FILES['upload']['error'])) // php 4.2 版本才支持 error
    {
        // 最大上传文件大小
        $php_maxsize = ini_get('upload_max_filesize');
        $htm_maxsize = '1M';
    ...
    else
    {
        ...
        if($_FILES['upload']['size'] / 1024 > $upload_size_limit)
        {
            ajax_show_message(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
        }
    }
    ...
    if (!empty($_FILES['upload']['name']))
    {
        ...
        $img_name = $user_id . '.' . end(explode('.', $_FILES['upload']['name']));

        $target = ROOT_PATH . DATA_DIR . '/upload/';

        $original_img = $image->upload_image($_FILES['upload'], 'upload', $img_name); // 原始图片

        $upload = $image->make_thumb($original_img, 190, '', $target);
    ...
    ajax_show_message($msg,'success',$upload);
