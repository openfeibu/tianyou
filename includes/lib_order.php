...
function cart_goods($type = CART_GENERAL_GOODS,$rec_id = [])
{
    ...
    if($rec_id)
    {
        $sql .= " AND " .db_create_in($rec_id, 'c.rec_id');
    }
...
function clear_cart($type = CART_GENERAL_GOODS,$rec_id)
{
    ...
    if($rec_id)
    {
        $sql .= " AND ".db_create_in($rec_id,"rec_id");
    }
    $GLOBALS['db']->query($sql);
}
 ...
 * @return  array
 */
function get_cart_goods($rec_type = CART_GENERAL_GOODS,$rec_id = [])
{
    /* 初始化 */
    $goods_list = array();
    $total = array(
...
{
    if (isset($_SESSION['flow_consignee']) && !$address_id)
    {
        ...
        if ($user_id > 0)
        {
            if($address_id >0)
            {
                $sql = "SELECT ua.*".
                        ...
                        " WHERE ua.user_id='$user_id' AND ua.address_id = '$address_id'";
            }else{
                /* 取默认地址 */
                $sql = "SELECT ua.*".
                        " FROM " . $GLOBALS['ecs']->table('user_address') . "AS ua ".
