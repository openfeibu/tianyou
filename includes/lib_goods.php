...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,filter_attr,grade,is_author,is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
                $cat_arr[$row['cat_id']]['name'] = $row['cat_name'];
                $cat_arr[$row['cat_id']]['filter_attr']   = $row['filter_attr'];
                $cat_arr[$row['cat_id']]['grade']   = $row['grade'];
                $cat_arr[$row['cat_id']]['is_author']   = $row['is_author'];
                $cat_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id,filter_attr,is_author,grade,is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
               ...
               $three_arr[$row['cat_id']]['grade']   = $row['grade'];
               $three_arr[$row['cat_id']]['is_author']   = $row['is_author'];
