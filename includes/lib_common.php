 ...
 * @return      array
 */
function get_regions($type = 0, $parent = 1)
{
...
function get_regions_province($parent = 1)
{
    $sql = "SELECT region_id as id ,region_name as name FROM " . $GLOBALS['ecs']->table('region') .
    ...
    foreach($data as $key => $value)
    {
        ...
        $data[$key ]['city'] = $GLOBALS['db']->GetAll($sql);
    }
    return $data;
}
function get_regions_area($parent = 1)
{
    $sql = "SELECT region_id as id,region_name as name,parent_id as pid FROM " . $GLOBALS['ecs']->table('region') .
            " where parent_id > '$parent'";
    return $GLOBALS['db']->GetAll($sql);
}
/**
...
{
    $sql = "SELECT author_name ,author_id FROM " . $GLOBALS['ecs']->table('author') ;
    $row = $GLOBALS['db']->getAll($sql);
    foreach ($row as $key => $value) {
        $row[$key]['autor_initial'] = get_initial($value['author_name']);
    }
    $arr = arr_regroup($row,'autor_initial');
    ksort($arr);
    return $arr;
}
...
function arr_regroup($arr,$field)
{
    $item = array();
    foreach($arr as $k=>$v){
        if(!isset($item[$v[$field]])){
            $item[$v[$field]][]=$v;
        }else{
            $item[$v[$field]][]=$v;
        }
...
function handle_authors($authors)
{
    $arr = array();
    $arr['filter_attr_name'] = '作者';
    $arr['special'] = '2';
    $arr['attr_list'] = $authors;
    return $arr;
}

function get_consignee_region($consignee,$separator='')
{
    // if(isset($consignee['country']))
    // {
    //     $sql = "select region_name from ".$GLOBALS['ecs']->table('region') . " where region_id in(".$consignee['country'].",".$consignee['province'].",". $consignee['city'].",".$consignee['district'].")";
    // }else{
        $sql = "select region_name from ".$GLOBALS['ecs']->table('region') . " where region_id in(".$consignee['province'].",". $consignee['city'].",".$consignee['district'].")";
    //}

    $address = $GLOBALS['db']->getAll($sql);
    $region ='';
    foreach($address as $value){
        $region .= $value['region_name'].$separator;
    }
    $region = rtrim($region, $separator);
    return $region;
}
function get_articles($cat_id, $size = 6 )
{
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $arr[$article_id]['area_size']   = $row['area_size'];
