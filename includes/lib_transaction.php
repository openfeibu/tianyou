...
function edit_profile($profile)
{
    ...
    {
        $cfg['country'] = $profile['country'];
    }
    if (!empty($profile['province']))
    {
        $cfg['province'] = $profile['province'];
    }
    if (!empty($profile['city']))
    {
        $cfg['city'] = $profile['city'];
    }
    if (!empty($profile['district']))
    {
        $cfg['district'] = $profile['district'];
    }
...
function get_profile($user_id)
{
    ...
    $info['avatar']      = isset($infos['avatar']) ? $infos['avatar'] : '';
    ...
    $info['region'] = get_consignee_region($infos,'-');
...
function get_user_orders($user_id, $num = 10, $start = 0,$keyword)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" class='fb-color-fff'>".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\"  class='fb-color-fff'>".$GLOBALS['_LANG']['received']."</a>";
            }
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                @$row['handler'] = "<a href='javascript:;' class='fb-color-fff'>" .$GLOBALS['_LANG']['ss_received']. '</a>';
            }
            else
            {
                if ($row['pay_status'] == PS_UNPAYED)
                {
                    @$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '" class="fb-color-fff">' .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    @$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '" class="fb-color-fff">' .$GLOBALS['_LANG']['view_order']. '</a>';
                }
        ...
        {
            $row['handler'] = "<a href='javascript:;' class='fb-color-fff'>" .$GLOBALS['_LANG']['os'][$row['order_status']]. '</a>';
        }
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'pay_status'     => $row['pay_status'], /* 退换货插件 加强版 */
                       'back_status'    => $GLOBALS['db']->getOne("SELECT status FROM " . $GLOBALS['ecs']->table('order_back') . " WHERE order_sn = '" .    $row['order_sn'] . "'"), /* 退换货插件 加强版 */
...
function update_address($address)
{
    ...
    {
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_address'), ['is_default' => 0], 'UPDATE', 'user_id = ' . $address['user_id']);
    }
