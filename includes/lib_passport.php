 ...
 * @return  bool         $bool
 */
function register($username, $password, $mobile, $other = array())
{
    ...
    if (empty($mobile))
    {
        $GLOBALS['err']->add($GLOBALS['_LANG']['mobile_empty']);
    }
    else
    {
        if (!is_mobile($mobile))
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['mobile_invalid'], htmlspecialchars($mobile)));
        }
    }
    ...
    if (!$GLOBALS['user']->add_user($username, $password, $mobile))
    {
        ...
        elseif ($GLOBALS['user']->error == ERR_MOBILE_EXISTS)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['mobile_exist'], $mobile));
        }elseif ($GLOBALS['user']->error == ERR_MOBILE_EMAIL)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['mobile_invalid'], $mobile));
        }elseif ($GLOBALS['user']->error == ERR_MOBILE_NOT_ALLOW)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['mobile_not_allow'], $mobile));
        }
        else
        {
