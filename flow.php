...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,$goods->buy_type))
        {
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    $json = new JSON;
    /*
    $province = get_regions_province();
    $province_json = $json->encode($province);
    $path = dirname(__FILE__).DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'province_json.js';
    if(!file_exists($path))
    {
        $myfile = fopen($path, "w");
    }
    file_put_contents($path,$province_json);

    $area = get_regions_area();
    $area_json = $json->encode($area);
    $path = dirname(__FILE__).DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'area_json.js';
    if(!file_exists($path))
    {
        $myfile = fopen($path, "w");
    }
    file_put_contents($path,$area_json);
    exit;
*/
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
...
elseif ($_REQUEST['step'] == 'ajax_drop_consignee')
{
    /*------------------------------------------------------ */
    //-- 删除收货人信息
    /*------------------------------------------------------ */
    ...
    else
    {
        $dataa = [
            'code' => 200,
            'messgae' => $_LANG['not_fount_consignee']
        ];
    }
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    //-- 订单确认
    /*------------------------------------------------------ */
    if($_REQUEST['shopping'] && $_SESSION['cart_rec_id'])
    {
        $rec_id = $_SESSION['cart_rec_id'];
        unset($_SESSION['cart_rec_id']);
    }else{
        $rec_id = $_POST['rec_id'];
        $_SESSION['cart_rec_id'] = $rec_id;
    }
    ...
    {
        ecs_header("Location: user.php?act=consignee\n");
        exit;
    }
    */
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        " WHERE session_id = '" . SESS_ID . "' " .
        "AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type'" ." AND ".db_create_in($rec_id, 'rec_id');
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'], '', '', 'warning');
    }
    ...
    {
        $cart_goods_stock = get_cart_goods(CART_GENERAL_GOODS,$rec_id);
    ...
    //var_dump(check_consignee_info($consignee, $flow_type));exit;
    /* 检查收货人信息是否完整 */
    if (!check_consignee_info($consignee, $flow_type))
    {
        /* 如果不完整则转向到收货人信息填写界面 */
    ...
    $_POST['shipping'] = isset($_POST['shipping']) ? intval($_POST['shipping']) : 2;
    $_POST['payment'] = isset($_POST['payment']) ? intval($_POST['payment']) : 1;
   ...
   /* 判断是不是实体商品 */
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                ...
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,deposit,buy_type) ".
