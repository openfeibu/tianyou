;(function(window, $) {
	...
	fbUi.monitorBottom = function(options){
		...
		function addEvent(){
			...
			var wInnerH = window.innerHeight || document.documentElement.clientHeight; // 设备窗口的高度（不会变）
    ...
    fbUi.waterfall = function(options){
		...
		$.each(fb.data,function(a,b){
			...
			var min_index = $.inArray(minInNumbers, heightArray);
	...
	fbUi.prototype.MouseDirection = function (opts) {
		    var $el = element[0],rectWidth,rectHeight;
		    ...
		    var calculate = function (element, e) {
		        ...
		        x4 = rect.left + rectWidth;
		        y4 = -(rect.top + rectHeight);

		        x0 = rect.left + rectWidth / 2;
		        y0 = -(rect.top + rectHeight / 2);
	...
	fbUi.prototype.magnifier = function(options){
		...
		elTop = $el.offset().top
		;
		$el.css({"width":fb.width,"height":fb.height});
