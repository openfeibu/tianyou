...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 列表页面 */
    assign_query_info();
    $smarty->display($_CFG['template_name'].'category_list.htm');
}
...
/*获取分类属性*/
if($_REQUEST['act'] == 'get_goods_type')
{
    $cat_id = intval($_POST['cat_id']);
    $top_cats = get_parent_cats($cat_id);
    foreach ($top_cats AS $val)
    {
        $top_cat_id = $val['cat_id'];
        $filter_attr = $val['filter_attr'];
    }
    $filter_attr_arr = explode(',',$filter_attr);
    $filter_attr_id  = $filter_attr_arr ? $filter_attr_arr[0] : 0;
    $attr_cat_id = $GLOBALS['db']->getOne('SELECT cat_id FROM ' .$ecs->table('attribute'). " WHERE attr_id = '$filter_attr_id' " );
    echo $attr_cat_id;
}
/*------------------------------------------------------ */
//-- 添加商品分类
...
if ($_REQUEST['act'] == 'add')
{
    ...
    /* 显示页面 */
    assign_query_info();
    $smarty->display($_CFG['template_name'].'category_info.htm');
}
...
if ($_REQUEST['act'] == 'move')
{
    ...
    /* 显示页面 */
    assign_query_info();
    $smarty->display($_CFG['template_name'].'category_move.htm');
}
