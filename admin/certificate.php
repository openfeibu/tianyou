...
if ($_REQUEST['act']== 'list_edit')
{
    ...
    if(intval(RELEASE)<intval($version['release'])){
        ...
        $message = "您现在已经是最新版本了,当前版本:TIANYOU ".EC_CHARSET." ".VERSION;
    }
    $smarty->assign('message',$message);
    $smarty->display($_CFG['template_name'].'certificate.htm');
}
...
elseif($_REQUEST['act']=='apply_bindrelation')
{
    ...
    $smarty->display($_CFG['template_name'].'apply_bindrelation.htm');
}
...
elseif($_REQUEST['act']=='accept_bindrelation')
{
    ...
    $str_url = MATRIX_REALTION_URL.implode("&", $array_params);
    $smarty->assign('str_url',$str_url);
    $smarty->display($_CFG['template_name'].'accept_bindrelation.htm');
}
...
elseif($_REQUEST['act']=='authority_url'){
    ...
    $smarty->display($_CFG['template_name'].'authority.htm');
}
