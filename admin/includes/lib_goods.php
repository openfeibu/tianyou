...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, a.special,v.attr_value, v.attr_price ".
...
function get_build_attr_html($cat_id, $goods_id = 0)
{
    $attr = get_attr_list($cat_id, $goods_id);
    $html = '';
    $spec = 0;
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        if ($val['attr_input_type'] == 0)
        {
            ...
            if($val['special'] == 1 && $special_number ==0){
                $html .= "<div class='input_all size'>";
                $html .= '<label for="">尺寸</label>';

            }
            $html .= '<input type="text" name="attr_value_list[]" placeholder="'.$val[attr_name].'" value="'.$val['attr_value'].'">';
            if($val['special'] == 1 && $special_number >0){
                $html .= '</div>';
            }
            else{
                $html .= '<input type="text" class="ride" disabled="" placeholder="x">';
            }
            $special_number++;
        }
        elseif ($val['attr_input_type'] == 2)
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['nature']           = empty($_REQUEST['nature']) ? 0 : intval($_REQUEST['nature']);
        ...
        if ($filter['nature'])
        {
            $where .= " AND (nature = '" . $filter['nature'] . "')";
        }
