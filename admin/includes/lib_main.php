...
function sys_msg($msg_detail, $msg_type = 0, $links = array(), $auto_redirect = true)
{
    ...
    $GLOBALS['smarty']->display($_CFG['template_name'].'message.htm');
...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id,filter_attr FROM ' . $GLOBALS['ecs']->table('category'));
    ...
    while (1)
    {
        foreach ($arr AS $row)
        {
            if ($cat == $row['cat_id'])
            {
                ...
                $cats[$index]['filter_attr'] = $row['filter_attr'];
                $index++;
                break;
            }
        }
