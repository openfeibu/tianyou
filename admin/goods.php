...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $smarty->display($_CFG['template_name'].$htm_file);
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        $sql = "SELECT * FROM " . $ecs->table('author') . " WHERE author_id = '$goods[author_id]'";
        $author = $db->getRow($sql);
        if($author)
        {
            $goods['author_name'] = $author['author_name'];
            $goods['author_id'] = $author['author_id'];
        }
    }
    ...
    $smarty->display($_CFG['template_name'].'goods_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 处理商品图片 */
    $goods_img        = $_POST['goods_img'];  // 初始化商品图片
    $goods_thumb      = $_POST['goods_thumb'];  // 初始化商品缩略图
    $original_img     = $_POST['original_img'];  // 初始化原始图片
    ...
    $nature = !empty($_POST['nature']) ? intval($_POST['nature']): 0;
    /* 入库 */
    if ($is_insert)
    {
        if ($code == '')
        {
            ...
            "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,year,rent,deposit,nature)" .
                ...
                "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$year','$rent','$deposit','$nature')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,year,rent,deposit,nature)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$year','$rent','$deposit','$nature')";
        }
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'show_image')
{
    ...
    $smarty->display($_CFG['template_name'].'goods_show_image.htm');
}
...
elseif ($_REQUEST['act'] == 'get_attr')
{
    ...
    $content    = get_build_attr_html($goods_type, $goods_id);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    $smarty->display($_CFG['template_name'].'product_info.htm');
}
